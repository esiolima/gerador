...
for (const dir of [UPLOAD_DIR, LOGOS_DIR]) {
...
const uploadExcel = multer({
  ...
  fileFilter: (req, file, cb) => {
    if (!file.originalname.endsWith(".xlsx")) {
      return cb(new Error("Arquivo inválido: apenas planilhas .xlsx são permitidas."));
    }
    cb(null, true);
  },
});
...
const uploadLogo = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    if (!allowedTypes.includes(file.mimetype)) {
      return cb(new Error("Arquivo inválido: apenas imagens PNG, JPG, JPEG, WEBP e SVG são permitidas."));
    }
...
export function setupUploadRoute(app: express.Application) {
  ...
  app.use("/api/upload", (err: Error, req: Request, res: Response, next: express.NextFunction) => {
