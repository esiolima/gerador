...
export class CardGenerator extends EventEmitter {
  ...
  async initialize() {
    ...
    this.browser = await puppeteer.launch({
      executablePath:
        process.env.PUPPETEER_EXECUTABLE_PATH || process.env.CHROME_BIN || "/usr/bin/chromium",
  ...
  private validateRows(rows: any[]): void {
    if (!rows.length) throw new Error("A planilha não possui linhas de dados para processar.");
    const headers = Object.keys(rows[0] ?? {}).map((h) => h.toLowerCase().trim());
    const missing = ["tipo"].filter((h) => !headers.includes(h));
    if (missing.length) throw new Error(`Coluna obrigatória ausente: ${missing.join(", ")}. Use nomes em minúsculo.`);
    rows.forEach((row, index) => {
      const line = index + 2;
      const tipo = this.normalizeType(row.tipo);
      if (!String(row.tipo ?? "").trim()) throw new Error(`Erro na linha ${line}: a coluna "tipo" está vazia.`);
      if (!tipo || !VALID_TYPES.includes(tipo)) throw new Error(`Erro na linha ${line}: tipo "${row.tipo}" não reconhecido. Use promocao, cupom, cashback ou queda.`);
      if (tipo === "cupom" && !String(row.cupom ?? "").trim()) throw new Error(`Erro na linha ${line}: template cupom exige a coluna "cupom" preenchida.`);
      if (!String(row.valor ?? "").trim()) throw new Error(`Erro na linha ${line}: a coluna "valor" está vazia.`);
    });
  }
...
<style id="fit-container-helpers">
  .logo, .selo, .valor-container, .cupom-codigo { overflow: hidden; }
  .logo img, .selo img, #selo-img { width: 100%; height: 100%; object-fit: contain; display: block; }
  .texto-topo, .complemento, .legal, .uf, .segmento, .segmento-wrapper { text-align: center; }
  #valor-texto, .valor-texto, #cupom-text { text-rendering: geometricPrecision; }
</style>
<script>
(function(){
  function fitText(el, container, opts){
    if(!el || !container) return;
    var max = opts.max || 480, min = opts.min || 10;
    el.style.display = 'block';
    el.style.maxWidth = '100%';
    el.style.textAlign = 'center';
    el.style.overflow = 'visible';
    el.style.whiteSpace = opts.nowrap ? 'nowrap' : 'normal';
    el.style.wordBreak = opts.breakWords ? 'break-word' : 'normal';
    el.style.overflowWrap = opts.breakWords ? 'anywhere' : 'normal';
    el.style.lineHeight = opts.lineHeight || '0.92';
    el.style.fontSize = max + 'px';
    for(var size=max; size>=min; size--){
      el.style.fontSize = size + 'px';
      if(el.scrollWidth <= container.clientWidth && el.scrollHeight <= container.clientHeight) break;
    }
  }
  function run(){
    fitText(document.getElementById('valor-texto') || document.querySelector('.valor-texto'), document.getElementById('valor-container') || document.querySelector('.valor-container'), {max: 520, min: 22, nowrap: false, breakWords: false, lineHeight:'0.9'});
    fitText(document.getElementById('cupom-text'), document.querySelector('.cupom-codigo'), {max: 116, min: 18, nowrap: true, breakWords: false, lineHeight:'1'});
  }
  ...
  private replacePlaceholders(html: string, row: any, tipo: string, logoBase64: string, seloBase64: string): string {
  ...
  async generateCards(excelFilePath: string, originalFileName?: string): Promise<GenerateCardsResult> {
    ...
    for (const [index, row] of rows.entries()) {
      ...
      if (!fs.existsSync(templatePath)) throw new Error(`Template não encontrado: templates/${tipo}.html`);
      ...
      const seloBase64 = seloRaw ? this.imageToBase64(path.join(SELOS_DIR, seloRaw === "nova" ? "acaonova.png" : seloRaw === "renovada" ? "acaorenovada.png" : "blank.png")) : "";

      let html = fs.readFileSync(templatePath, "utf8");
      html = this.injectFittingHelpers(this.replacePlaceholders(html, row, tipo, logoBase64, seloBase64));

      const tmpHtmlPath = path.join(TMP_DIR, `${jobId}_card_${index + 1}.html`);
      fs.writeFileSync(tmpHtmlPath, html);

      const page = await this.browser.newPage();
      await page.setViewport({ width: 700, height: 1058, deviceScaleFactor: 2 });
      ...
      const ordemFinal = row.ordem && String(row.ordem).trim() !== "" ? String(row.ordem).trim() : String(index + 1);
      ...
      const pdfName = `${safeOrdem}_${tipo}_${categoria}.pdf`;
      const pngName = `${safeOrdem}_${tipo}_${categoria}.png`;
      const pdfPath = path.join(jobDir, pdfName);
      const pngPath = path.join(jobDir, pngName);

      await page.pdf({ path: pdfPath, width: "700px", height: "1058px", printBackground: true, margin: { top: "0px", right: "0px", bottom: "0px", left: "0px" } });
      ...
      this.emit("progress", { processed, total, percentage: Math.round((processed / total) * 100), currentCard: `${processed}/${total} cards processados` });
    }

    const baseName = originalFileName ? path.parse(originalFileName).name : path.parse(excelFilePath).name;
    ...
    archive.pipe(output);
    for (const card of cards) archive.file(path.join(jobDir, card.pdfName), { name: card.pdfName });
    await archive.finalize();
    await new Promise<void>((resolve, reject) => { output.on("close", () => resolve()); output.on("error", reject); });

    fs.writeFileSync(path.join(jobDir, "cards.json"), JSON.stringify({ jobId, cards }, null, 2));
    return { zipPath, zipName: path.basename(zipPath), jobId, cards, totalRows: total, processedRows: processed };
  }
